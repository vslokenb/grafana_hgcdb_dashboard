...
DS_UID_MAP = {
    "mac_postgres_db":                    GF_DS_UID,   # stocks LD dashboard
...
def upload_folder(folder_name: str, json_dir: str):
    ...
    for filename in sorted(os.listdir(json_dir)):
        ...
        # Strip server-assigned id so Grafana accepts it as create-or-overwrite
        dashboard.pop("id", None)

        dashboard = remap_datasource_uids(dashboard, DS_UID_MAP)

        client.upload_dashboard_json(dashboard, uid)


if not os.path.isdir(STATIC_DIR):
    print(f"[Static] No static_dashboards/ directory found — skipping.")
    sys.exit(0)
